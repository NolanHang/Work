var beihai_rigion = 
    ...
    Map.centerObject(beihai_rigion, 4);
    var box = Beibu_Gulf;
    var startYear = 1986; 
    var endYear = 2021; 
    ...
    // for(var pp = 0;pp<number_index;pp++){
      ...
      var new_disturb_gain = min_disturb.select([1]).updateMask(max_mask).updateMask(min_mask).updateMask(min_disturb.select([2]).gt(0)).clip(mangrove_2019);
      var new_disturb_loss = min_disturb.select([1]).updateMask(max_mask).updateMask(min_mask).updateMask(min_disturb.select([2]).lt(0)).clip(mangrove_2019);
    ...
    var all_annual_image_unmask_array0 =all_annual_image_unmask_array.arrayMask(mask_array);//先去除被掩模的部分，使所有空格排在后面
    var length1 = all_annual_image_unmask_array0.arrayLength(0);
    var all_annual_image_unmask_array9999 = all_annual_image_unmask_array0.arrayPad([36],8888);//填充后面几年的空格年份
    ...
    // Map.addLayer(image_shape,{},'image_shape');
    ...
    var Z_increace = Z_1.divide(DX.sqrt()).clip(mangrove_2019);
    var Z_decreace = Z_2.divide(DX.sqrt()).clip(mangrove_2019);
